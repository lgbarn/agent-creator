...
# Checks: package.json, .claude-plugin/plugin.json, .claude-plugin/marketplace.json
#
...
if [[ "${PLUGIN_VERSION}" != "${PKG_VERSION}" ]]; then
    echo "ERROR: .claude-plugin/plugin.json version (${PLUGIN_VERSION}) != package.json (${PKG_VERSION})"
    ERRORS=$((ERRORS + 1))
fi

if [[ "${MKT_VERSION}" != "${PKG_VERSION}" ]]; then
    echo "ERROR: .claude-plugin/marketplace.json version (${MKT_VERSION}) != package.json (${PKG_VERSION})"
    ERRORS=$((ERRORS + 1))
fi
